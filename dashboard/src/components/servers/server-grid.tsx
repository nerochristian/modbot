...
export function ServerGrid({ guilds }: { guilds: ManagedGuild[] }) {
  ...
  async function openGuild(guildId: string) {
    ...
    try {
      ...
      toast.error("Server could not be opened", message);
    } finally {
      setOpening(null);
    }
  }
    ...
    <div className="rounded-3xl border border-border bg-surface/45 p-4 shadow-2xl shadow-black/10 backdrop-blur-xl sm:p-6">
      <div className="mb-6 flex flex-wrap items-center justify-between gap-3 border-b border-border pb-5">
        <p className="text-sm text-muted">
          <span className="font-semibold text-foreground">{installed}</span>{" "}
          connected ·{" "}
          <span className="font-semibold text-foreground">
            {guilds.length - installed}
          </span>{" "}
          available to add
        </p>
        <button
          ...
          onClick={async () => {
            setRefreshing(true);
            try {
              const response = await fetch("/api/guilds?refresh=1", {
                cache: "no-store",
              });
              const body = await response.json().catch(() => ({}));
              if (!response.ok)
                throw new Error(body.error ?? "Could not refresh servers");
              router.refresh();
            } catch (reason) {
              toast.error(
      ...
      {openError && (
        <div
          className="mb-5 flex items-start justify-between gap-3 border border-danger/30 bg-danger-soft px-3 py-2 text-sm text-danger"
          role="alert"
        >
      ...
      {guilds.length === 0 ? (
        <div className="rounded-2xl border border-dashed border-border-strong bg-surface p-10 text-center">
          ...
          <p className="mx-auto mt-2 max-w-md text-sm text-muted">
            Docket only shows servers you own or where your Discord account has
