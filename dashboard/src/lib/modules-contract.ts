...
export const MODULE_DEFINITIONS: readonly ModuleDefinition[] = [
  ...
  },
  {
    id: 'antiraid',
    name: 'Guardian',
    description: 'Total server defense: raid detection for join floods plus anti-nuke tripwires that stop rogue admins mass-deleting, mass-banning, or hijacking permissions.',
    category: 'Moderation',
    icon: 'shield-x',
    badge: 'standard',
    ...
    fields: [
      ...
      { key: 'antiraid_log_channel', label: 'Guardian log channel', type: 'channelId', channelTypes: [0, 5], hint: 'Raid detections and nuke alerts are recorded here. Falls back to the moderation log.' },
      ...
      { key: 'guardian_nuke_enabled', label: 'Anti-nuke protection', type: 'toggle', fallback: false, hint: 'Watch staff accounts for destructive bursts and stop them automatically. Server owner and bots are always exempt.' },
      {
        key: 'guardian_nuke_action', label: 'Anti-nuke response', type: 'select', fallback: 'strip', options: [
          { label: 'Strip all roles (recommended)', value: 'strip' },
          { label: 'Ban the offender', value: 'ban' },
          { label: 'Kick the offender', value: 'kick' },
          { label: 'Apply quarantine role', value: 'quarantine' },
        ],
        hint: 'Stripping roles removes every permission the offender has without removing them from the server.',
      },
      { key: 'guardian_nuke_quarantine_role', label: 'Anti-nuke quarantine role', type: 'roleId', hint: 'Required when the anti-nuke response is quarantine.' },
      { key: 'guardian_nuke_log_channel', label: 'Anti-nuke alert channel', type: 'channelId', channelTypes: [0, 5], hint: 'Nuke alerts are posted here. Falls back to the Guardian log channel.' },
      { key: 'guardian_nuke_cooldown_seconds', label: 'Anti-nuke cooldown', type: 'number', min: 10, max: 3600, fallback: 60, hint: 'Seconds before another anti-nuke response can run.' },
      { key: 'guardian_channel_delete_threshold', label: 'Channel deletes to trip', type: 'number', min: 1, max: 100, fallback: 3 },
      { key: 'guardian_channel_delete_window', label: 'Channel delete window (s)', type: 'number', min: 5, max: 600, fallback: 30 },
      { key: 'guardian_channel_create_threshold', label: 'Channel creates to trip', type: 'number', min: 1, max: 100, fallback: 5 },
      { key: 'guardian_channel_create_window', label: 'Channel create window (s)', type: 'number', min: 5, max: 600, fallback: 30 },
      { key: 'guardian_role_delete_threshold', label: 'Role deletes to trip', type: 'number', min: 1, max: 100, fallback: 3 },
      { key: 'guardian_role_delete_window', label: 'Role delete window (s)', type: 'number', min: 5, max: 600, fallback: 30 },
      { key: 'guardian_ban_threshold', label: 'Bans to trip', type: 'number', min: 1, max: 100, fallback: 5 },
      { key: 'guardian_ban_window', label: 'Ban window (s)', type: 'number', min: 5, max: 600, fallback: 30 },
      { key: 'guardian_kick_threshold', label: 'Kicks to trip', type: 'number', min: 1, max: 100, fallback: 5 },
      { key: 'guardian_kick_window', label: 'Kick window (s)', type: 'number', min: 5, max: 600, fallback: 30 },
      { key: 'guardian_webhook_create_threshold', label: 'Webhooks to trip', type: 'number', min: 1, max: 100, fallback: 2 },
      { key: 'guardian_webhook_create_window', label: 'Webhook window (s)', type: 'number', min: 5, max: 600, fallback: 30 },
      { key: 'guardian_dangerous_grant_threshold', label: 'Permission grants to trip', type: 'number', min: 1, max: 100, fallback: 1, hint: 'Grants of admin-level permissions (admin, manage server/roles/channels, ban, kick, webhooks). One grant trips by default.' },
      { key: 'guardian_dangerous_grant_window', label: 'Permission grant window (s)', type: 'number', min: 5, max: 600, fallback: 60 },
      { key: 'guardian_watch_channel_delete', label: 'Watch channel deletions', type: 'toggle', fallback: true },
      { key: 'guardian_watch_channel_create', label: 'Watch channel creations', type: 'toggle', fallback: true },
      { key: 'guardian_watch_role_delete', label: 'Watch role deletions', type: 'toggle', fallback: true },
      { key: 'guardian_watch_ban', label: 'Watch bans', type: 'toggle', fallback: true },
      { key: 'guardian_watch_kick', label: 'Watch kicks', type: 'toggle', fallback: true },
      { key: 'guardian_watch_webhook_create', label: 'Watch webhook creations', type: 'toggle', fallback: true },
      { key: 'guardian_watch_dangerous_grant', label: 'Watch permission grants', type: 'toggle', fallback: true },
      { key: 'guardian_ignored_roles', label: 'Trusted roles (bypass)', type: 'roleIds', hint: 'Members with these roles never trip anti-nuke tripwires.' },
    ],
  },
  {
    id: 'appeals',
  ...
  },
  {
    ...
    fields: [
      { key: 'verification_method', label: 'Verification method', type: 'select', fallback: 'discord', hint: 'Use Docket inside Discord, or send members to the protected website check.', options: [{ label: 'Discord captcha', value: 'discord' }, { label: 'Website · reCAPTCHA', value: 'website' }] },
  ...
  },
  {
    ...
    fields: [
      ...
      { key: 'welcome_bg_url', label: 'Background image URL', type: 'url', placeholder: 'https://…/bg.png', hint: 'Leave empty to use each member’s own banner (or a soft accent gradient).' },
      { key: 'welcome_card_blur', label: 'Background blur', type: 'number', min: 0, max: 40, fallback: 12, hint: '0 keeps the image sharp; higher melts it into a backdrop.' },
      { key: 'welcome_card_overlay', label: 'Background dim %', type: 'number', min: 0, max: 90, fallback: 55, hint: 'How dark the background sits behind the text.' },
      { key: 'welcome_card_layout', label: 'Card layout', type: 'select', fallback: 'center', options: [{ label: 'Centered', value: 'center' }, { label: 'Left avatar', value: 'left' }] },
      { key: 'welcome_card_label', label: 'Welcome label', type: 'text', maxLength: 24, placeholder: 'WELCOME', hint: 'The small pill above the name.' },
      ...
      { key: 'welcome_card_role_color', label: 'Accent from member’s role color', type: 'toggle', fallback: false, hint: 'Overrides the accent color with each member’s top role color.' },
      { key: 'welcome_card_badges', label: 'Show Discord badges', type: 'toggle', fallback: true },
      { key: 'welcome_card_member_count', label: 'Show member count', type: 'toggle', fallback: true },
    ],
  },
  {
    id: 'autoroles',
