...
test('production SQL and standalone deployment match the bot runtime contracts', () => {
  ...
  assert.match(modulesService, /if \(enabled && def\.id === 'verification'\) \{[\s\S]*changes\.autoroles_enabled = false/)
  assert.match(modulesService, /changes\.verification_method = 'discord'/)
  ...
  assert.match(modulesClient, /useState<VerificationMethod>\('discord'\)/)
  assert.match(modulesClient, /Discord CAPTCHA/)
  assert.doesNotMatch(modulesClient, /Website checkpoint/)
...
test('public verification uses an explicit Cloudflare Turnstile flow', () => {
  ...
  assert.match(verificationRoute, /TURNSTILE_SECRET_ERRORS/)
  assert.match(verificationRoute, /invalid-input-secret/)
  ...
  assert.match(verificationRoute, /body: JSON\.stringify\(\{ roles: finalRoles \}\)/)
  assert.match(verificationRoute, /updatedRoles\.has\(unverifiedRole\)/)
  assert.match(verificationRoute, /sendBotChannelEmbed\(logChannel/)
  assert.match(verificationRoute, /title: 'Verification complete'/)
  assert.match(verificationRoute, /thumbnail: \{ url: avatarUrl \}/)
  assert.match(verificationRoute, /footer: \{ text: `@\$\{username\}`/)
  assert.match(verificationRoute, /reservedNonce && !rolePatchAccepted/)
  assert.doesNotMatch(verificationRoute, /passed website verification/)
  assert.doesNotMatch(verificationRoute, /members\/\$\{capability\.u\}\/roles\/\$\{verifiedRole\}/)
