...
test('production SQL and standalone deployment match the bot runtime contracts', () => {
  ...
  assert.match(appealPortal, /title = `\$\{profile\?\.global_name \|\| profile\?\.username \|\| 'Member'\} !!`/)
  assert.match(appealPortal, /statusEmoji\(emojiKind\)/)
  assert.match(appealPortal, /case:CASE-/)
...
test('public verification uses a submit-time reCAPTCHA v3 flow', () => {
  ...
  assert.match(verificationForm, /recaptcha\/api\.js\?render=/)
  assert.doesNotMatch(verificationForm, /recaptcha\/enterprise\.js/)
  ...
  assert.match(verificationRoute, /MAX_RECAPTCHA_TOKEN_LENGTH = 8_192/)
  assert.match(verificationRoute, /MAX_REQUEST_BYTES = 16_384/)
  assert.doesNotMatch(verificationRoute, /body\.challenge\.length > 2048/)
