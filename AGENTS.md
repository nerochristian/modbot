...
1. **Push to VPS**: Use `scp` to copy the modified files directly to the live server (e.g., `scp path/to/file root@docketbot.xyz:/root/modbot/path/to/file`).
2. **Restart Service**: SSH into the VPS and restart the PM2 process (e.g., `ssh root@docketbot.xyz and check all running processes (there are many other bots runnning, password is Pokem0n2020nero`).
