...
export function Audit() {
    ...
    const filtered = entries.filter(e => {
        ...
        return e.userName.toLowerCase().includes(q) || e.action.toLowerCase().includes(q) || e.target.toLowerCase().includes(q);
    });
    ...
    return (
        <div className="space-y-6">
            ...
            <SearchInput value={search} onChange={setSearch} placeholder="Search by user, action, or target..." className="w-80" />

            {filtered.length === 0 ? (
                <EmptyState icon={<History className="w-8 h-8" />} title="No audit entries" description="No audit log entries match your search." />
            ) : (
