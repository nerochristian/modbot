...
class Setup(commands.Cog):
    ...
    async def setup(
        ...
        for cfg in roles_to_create:
              try:
                  # 1. Determine intended name and icon based on server features
                  final_name = cfg["name"]
                  final_icon = None
                  ...
                  if icon_bytes:
                      # Check for Role Icons feature (Level 2 Boost or other feature flag)
                      if "ROLE_ICONS" in guild.features:
                           final_icon = icon_bytes
                      else:
                           # Fallback: Create/Find emoji and prepend to name
                           ...
                           if not emoji:
                               try:
                                   emoji = await guild.create_custom_emoji(
                                       name=emoji_name, 
                                       image=icon_bytes, 
                                       reason="ModBot Setup: Role Icon Fallback"
                                   )
                               except Exception:
                                   pass
                  ...
                  # If not found by ID, try finding by original name
                  if not existing:
                      existing = discord.utils.get(guild.roles, name=cfg["name"])
                  
                  # If still not found, try finding by fallback name (if applicable)
                      ...
                      if cfg.get("setting_key") not in non_staff_roles and cfg.get("setting_key") != "owner_role":
                          try:
                              await existing.edit(
                                  name=final_name, 
                                  display_icon=final_icon, 
                                  reason="ModBot Setup: update role icon"
                              )
                          except Exception:
                              pass

                      settings[cfg["setting_key"]] = existing.id
                      created_roles.append(f"✅ {existing.mention} (already exists)")
                  else:
