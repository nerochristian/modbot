...
class TempbanExpiryTests(unittest.IsolatedAsyncioTestCase):
    async def test_expired_query_accepts_legacy_iso_timestamps(self):
        ...
        self.assertEqual([entry["user_id"] for entry in expired], [100])

    async def test_expired_tempban_is_unbanned_logged_and_sent_rejoin_dm(self):
        ...
        moderation.create_mod_embed = AsyncMock(
            return_value=discord.Embed(title="Member automatically unbanned")
        )
    ...
    async def test_failed_unban_stays_queued_for_retry(self):
